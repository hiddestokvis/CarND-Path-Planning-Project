...
double changeSpeed(double v, double target) {
  return (double) v * 0.92 + target * 0.08;
}
            ...
            for (int i = 0; i < sensor_fusion.size(); i += 1) {
              ...
              if (d < (2 + 4 * lane + 2) && d > (2 + 4 * lane - 2)) {
                if ((check_car_s > car_s) && ((check_car_s - car_s < 30))) {
                  too_close = true;
                  target_speed = check_speed * 2.24 - 10;
                }
              } else if(d < (2 + 4 * (lane - 1) + 2) && d > (2 + 4 * (lane - 1) - 2)) { // Left lane
                if (lane > 0 && distance < LCL) {
                  LCL = distance;
                }
              } else { // Right lane
                if (lane < 2 && distance < LCR) {
                  LCR = distance;
                }
              }
            }
              ...
              if (((LCL > 15 && LCL < 1000) || (LCR > 15 && LCR < 1000))) {
                ...
                if (distanceFromLane < 0.5) {
                  if (LCR == 1000) { // Prefer left shift
                    cout << "LANE SHIFT LEFT " << distanceFromLane << endl;
                    lane -= 1;
                    cout << "LANE: " << lane << endl;
                  } else if (LCL == 1000) {
                    cout << "LANE SHIFT RIGHT " << distanceFromLane << endl;
                    lane += 1;
                    cout << "LANE: " << lane << endl;
                  } else if (LCL > LCR){
                    cout << "LANE SHIFT LEFT " << distanceFromLane << endl;
